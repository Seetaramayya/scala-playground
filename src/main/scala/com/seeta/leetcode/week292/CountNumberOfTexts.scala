...
/** Alice is texting Bob using her phone. The mapping of digits to letters is shown in the figure below. In order to add
  * a letter, Alice has to press the key of the corresponding digit `i` times, where `i` is the position of the letter
  * in the key.
  *
...
object CountNumberOfTexts {
  ...
  val keyMapping: Map[String, String] = keyboard.flatMap { case (key, value) =>
    value.zipWithIndex.map { case (ch, i) =>
      s"$key" * (i + 1) -> s"$ch"
    }
  }
