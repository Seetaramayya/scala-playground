...
/** https://leetcode.com/problems/largest-3-same-digit-number-in-string/
  *
  * You are given a string `input` representing a large integer. An integer is good if it meets the following
  * conditions:
  *
  ...
  *
  * '''Note:''' A substring is a contiguous sequence of characters within a string. There may be leading zeroes in num
  * or a good integer.
  *
  * ===Example 1:===
  * {{{
  ...
  *
  * Started : 2022-05-08T07:20:54.261609Z End : 2022-05-08T07:52:28.450830Z Duration: 32 mins
  *
  * ===Result===
  *
  * {{{
...
object LargestGoodInteger {
  ...
  val allCharsSame: String => Boolean = input =>
    input.length match {
      case 0 => false
  ...
  def largestGoodInteger(input: String): String = {
    if (!isCorrectLength(input) || isNotNumber(input)) {
      throw new IllegalArgumentException(
        "Input should only contains digits and length should be in between 3 and 1000 inclusive"
      )
    } else {
